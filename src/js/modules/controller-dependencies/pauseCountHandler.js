...
function pauseCountHandler(inputValuesArr) {
    ...
    if (activeBlock === "until") {
        Logic.setUntilPauseTime(new Date().toISOString()); // setting the time when Until was paused
    ...
    Visual.updateTitle(undefined, "pause"); // putting the pause icon in the document title
